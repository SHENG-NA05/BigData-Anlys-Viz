...
| :--- | :--- | :--- | :--- |
| **SA (系統分析)**<br>*(專案負責人)* | 系統規格制定、API 及 DB 設計、時程與分工協調 | `系統規格書.md`<br>`任務分工表.md`<br>API 規格定義表 | 所有成員 |
...
- [ ] **C-3：館藏導入 API 路由**
  - 實作上傳檔案端點，將檔案接收後拋轉給 DB 實作的資料解析函數。
- [ ] **C-4：企劃書編輯與匯出端點**
...
- [ ] **D-1：資料庫配置與 Schema 遷移**
  - 安裝設定 PostgreSQL，並以 Alembic 初始化資料庫遷移機制。
  - 建立 `users`、`catalog_books`、`curation_themes`、`proposals`、`cost_benefit_logs` 等資料表。
- [ ] **D-2：館藏 Excel/CSV 檔案解析與批次寫入**
  - 撰寫 Python 解析 Excel/CSV 內容的邏輯 (使用 `openpyxl` 或 `pandas`)。
  - 實作批次寫入資料庫函數 (使用 SQLAlchemy `bulk_insert_mappings` 或原生 SQL COPY)，優化萬筆圖書寫入效能。
- [ ] **D-3：索引優化與效能調校**
  - 針對 `catalog_books` 的 `isbn`、`classification_no` 與書名欄位建立索引，以加速比對。
- [ ] **D-4：戰情室數據聚合查詢**
  - 撰寫高效能 SQL Aggregation 查詢，依月、季、年統計累計節省的工時與經費。

### 5. AI (AI功能開發)
- [ ] **E-1：Gemini / OpenRouter API 串接**
  - 整合 SDK，設計與實作 `ai_service.py` 連線管理與錯誤重試機制。
- [ ] **E-2：Prompt 模板設計與調校**
  - 設計「主題大綱生成」與「企劃草案擴寫」的系統 Prompt，限制輸出格式必須為穩定的 JSON 結構。
- [ ] **E-3：館藏與主題語意匹配**
  - 實作篩選邏輯，依據 AI 生成的策展主題關鍵字或分類，在資料庫中匹配適合的館藏圖書。
  - （選配）導入輕量級語意比對，比對書籍簡介與策展大綱的關聯度。
